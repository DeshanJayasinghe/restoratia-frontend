<script lang="ts" setup>
  ...
  const submit = async (action: 'send_to_kitchen' | 'hold_order' | 'pay_and_fire') => {
  ...
  const placeOrder = async (action: 'send_to_kitchen' | 'hold_order' | 'pay_and_fire'): Promise<boolean> => {
    try {
      const params = {
      ...
      if (action == 'pay_and_fire') {
        emit('store-payment', response.data.body.order_id)
...
<template>
  <VRow style="height: 91vh;">
    ...
    <VCol cols="12" md="5">
      <Actions :form="form" :meta="meta" @on-click-action="(action:string)=>$emit('on-click-action',action)" />
      ...
      <VRow class="mt-2" dense>
        ...
        <VCol v-if="!isEditMode" cols="12" :md="canReceivePayment && data.orderType?.id != 'dine_in'?6:12">
          <VBtn
            block
            color="primary"
